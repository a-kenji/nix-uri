...
let
  ...
  fuzz =
    ((self.inputs.crane.mkLib pkgs).overrideToolchain rust.rustNightlyToolchainTOML).buildPackage
      (
        commonArgs
        // rec {
          inherit src;
          cargoArtifacts =
            ((self.inputs.crane.mkLib pkgs).overrideToolchain rust.rustNightlyToolchainTOML).buildDepsOnly commonArgs
            // {
